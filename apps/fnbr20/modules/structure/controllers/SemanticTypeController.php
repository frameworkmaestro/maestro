...
class SemanticTypeController extends MController
{
    ...
    {
        $structure = MApp::getService('fnbr20', '', 'structuresemantictype');
        if ($this->data->id == '') {
    ...
    public function formNewSemanticType(){
        ...
        $this->data->save = "@fnbr20/structure/semantictype/newSemanticType|formNewSemanticType";
        $this->data->close = "!$('#formNewSemanticType_dialog').dialog('close');";
        $this->data->title = _M('New SemanticType');
        $this->render();
    }
    ...
    {
        try {
            $model = new SemanticType($this->data->semantictype->idSemanticType);
