...
EOT;
    ...
    public function mmenuitem($control)
    {
        ...
        $control->options->iconCls = $control->property->icon ? : $this->glyphclass($control);
        MAction::generate($control);
    ...
    public function mmenubaritem($control)
    {
        ...
        $control->options->iconCls = $control->property->icon;
...
EOT;
    ...
    public function mpanel($control)
    {
        ...
        $toolbar = '';
        if ($control->property->toolbar) {
            $control->property->toolbar->id = $control->property->id > '_toolbar';
            $control->options->tools = "#{$control->toolbar->id}";
            $toolbar = $this->mdiv($control->property->toolbar);
        }
...
EOT;
    ...
    public function mplain($control){
        $color = $control->style->color ?: "black";
        return <<<EOT
<div style="color:{$color}">{$control->property->text}</div>
EOT;
