...
class PersistentCriteria extends BaseCriteria
{
    ...
    public function addClass($className, $alias = '', $classMap = NULL)
    {
        ...
        if (strrpos($className, '\\') === false) {
            ...
            $fullClassIndex = $this->classMap->getNamespace() . '\\' . $classIndex;
        }
        if ((!isset($this->classes[$fullClassIndex])) || ($alias != '')) {
            $this->classes[$fullClassIndex] = array($fullClassIndex, $alias);
        }
        if (!$classMap) {
            $classMap = $this->manager->getClassMap($fullClassIndex);
        }
        if ($alias != '') {
            $this->registerAlias($alias, $classMap);
        }
        if ($classMap) {
            $this->maps[$classIndex] = $classMap;
            $this->maps[$fullClassIndex] = $classMap;
        }
        return $this;
    }
    ...
    {
        if ($class instanceof \Maestro\Persistence\Map\ClassMap) {
